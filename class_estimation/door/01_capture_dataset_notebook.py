"""굴착기 Door 분류 데이터셋 구축 — Flask 캡처 서버 (노트북 현장 취득용)

Windows 노트북 + ZED 2i 환경에서 현장 도어 데이터를 취득한다.
camera_utils.py의 ZED SDK 카메라 자동 감지를 사용.

설치:
    1) ZED SDK 설치 (stereolabs.com)
    2) Python 3.10+ 설치 (python.org, Add to PATH 체크)
    3) ZED Python API 설치: "C:\\Program Files (x86)\\ZED SDK\\get_python_api.py" 실행
    4) setup_notebook.bat 실행
실행:
    run_notebook.bat 더블클릭
    또는: python 01_capture_dataset_notebook.py
...
def init_directories():
    ...
    if not info_path.exists():
        cam_name = camera.camera_type if camera else "unknown"
        info = {
