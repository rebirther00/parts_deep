"""
굴착기 도어 분류 모델 학습 스크립트 (Edge-only 버전)
- RGB 이미지를 Canny Edge로 변환하여 shape/구조 기반 분류
- Edge 3채널(E,E,E) + Aux(물리 치수) ResNet18 Transfer Learning
- Texture 의존을 완전히 제거하여 같은 종류의 다른 개체 간 일반화 향상
...
from edge_utils import (
    EdgeAuxResNet18, EdgeTransform, EdgeDataset, EDGE_IN_CHANNELS,
    NUM_AUX_FEATURES,
)
...
LOG_PATH = setup_logging("02_door_classification_edge_5090")
...
MODEL_SAVE_PATH = os.path.join(ARTIFACTS_DIR, "best_door_edge_model_5090.pth")
ONNX_SAVE_PATH = os.path.join(ARTIFACTS_DIR, "best_door_edge_model_5090.onnx")
...
train_transform = EdgeTransform(IMAGE_SIZE, is_train=True)
val_transform = EdgeTransform(IMAGE_SIZE, is_train=False)

print(f"Edge 전처리 설정 완료:")
print(f"  입력 크기: {IMAGE_SIZE}x{IMAGE_SIZE}, 채널: {EDGE_IN_CHANNELS} (E,E,E)")
print(f"  Train: Letterbox + Pad + Rotation(±15°) + Scale(90-110%) + Canny(변동)")
print(f"  Val: Letterbox + Pad + Canny(80,200) 고정")
print(f"  보조 피처: {NUM_AUX_FEATURES}개 (물리 치수 mm)")
print(f"  색상 증강 없음 (Edge는 texture-free)")
...
train_dataset = EdgeDataset(train_paths, train_labels, transform=train_transform)
...
test_loader = None
if not FULL_TRAIN:
    test_dataset = EdgeDataset(test_paths, test_labels, transform=val_transform)
...
def create_resnet_model(num_classes, pretrained=True):
    """Edge 3채널 + 보조 피처 입력 ResNet18 모델 생성"""
    return EdgeAuxResNet18(num_classes, pretrained=pretrained)
...
print(f"EdgeAuxResNet18 모델 로드 완료 (ImageNet 가중치, Edge(E,E,E) 3채널, Aux: {NUM_AUX_FEATURES}피처)")
...
try:
    ...
    model_cpu.eval()
    dummy_images = torch.randn(1, EDGE_IN_CHANNELS, IMAGE_SIZE, IMAGE_SIZE)
    ...
    torch.onnx.export(
        ...
        input_names=['edge_image', 'aux_features'],
        output_names=['output'],
        dynamic_axes={
            'edge_image': {0: 'batch_size'},
