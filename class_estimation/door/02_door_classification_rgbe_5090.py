"""
굴착기 도어 분류 모델 학습 스크립트 (RGBE 버전)
- RGB + Canny Edge 4채널 입력으로 texture + shape 동시 활용
- RGBD와 동일한 4채널 아키텍처, D 대신 E(Edge) 사용
...
from depth_utils import RGBDAuxResNet18, IN_CHANNELS, NUM_AUX_FEATURES
...
LOG_PATH = setup_logging("02_door_classification_rgbe_5090")
...
MODEL_SAVE_PATH = os.path.join(ARTIFACTS_DIR, "best_door_rgbe_model_5090.pth")
ONNX_SAVE_PATH = os.path.join(ARTIFACTS_DIR, "best_door_rgbe_model_5090.onnx")
TRAIN_INDICES_PATH = os.path.join(ARTIFACTS_DIR, "training_indices_door_rgbe_5090.json")
...
class_names_path = os.path.join(ARTIFACTS_DIR, "class_names_door_rgbe_5090.json")
...
train_transform = RGBETransform(IMAGE_SIZE, is_train=True)
val_transform = RGBETransform(IMAGE_SIZE, is_train=False)

print(f"RGBE 전처리 설정 완료:")
print(f"  입력 크기: {IMAGE_SIZE}x{IMAGE_SIZE}, 채널: {RGBE_IN_CHANNELS} (R,G,B,E)")
print(f"  Train: Letterbox + Pad + Rotation(±15°) + Scale + ColorJitter(RGB) + Canny(변동)")
...
test_loader = None
if not FULL_TRAIN:
    test_dataset = RGBEDataset(test_paths, test_labels, transform=val_transform)
...
def create_resnet_model(num_classes, pretrained=True):
    """RGBE 4채널 + 보조 피처 입력 ResNet18 모델 생성 (RGBDAuxResNet18 재사용)"""
...
print(f"RGBDAuxResNet18 모델 로드 완료 (RGB: ImageNet 가중치, E: 평균 초기화, Aux: {NUM_AUX_FEATURES}피처)")
...
try:
    ...
    torch.onnx.export(
        ...
        input_names=['rgbe_image', 'aux_features'],
        output_names=['output'],
        dynamic_axes={
            'rgbe_image': {0: 'batch_size'},
