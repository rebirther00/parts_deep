"""굴착기 Door 실시간 분류 추론 서버 (Edge-only 버전)

ZED X Mini 카메라 + Canny Edge + PyTorch 모델을 사용하여
도어를 실시간 분류하고 결과를 웹에 표시한다.
RGB → Canny Edge 변환 후 추론하여 texture 의존 없이 shape 기반 분류.

실행:
    python 04_door_realtime_inference_edge.py
    python 04_door_realtime_inference_edge.py --model artifacts/best_door_edge_model_5090.pth
...
from depth_utils import (
    MAX_DEPTH_MM, compute_aux_features, DEFAULT_INTRINSICS,
)
from edge_utils import (
    EdgeAuxResNet18, EdgeTransform, EDGE_IN_CHANNELS, CANNY_LOW, CANNY_HIGH,
)
...
parser.add_argument(
    "--model", type=str,
    default=os.path.join(ARTIFACTS_DIR, "best_door_edge_model_5090.pth"),
    help="모델 파일 경로 (기본: artifacts/best_door_edge_model_5090.pth)",
)
parser.add_argument(
...
class PyTorchEdgeInferenceEngine:
    """PyTorch Edge + 보조 피처 추론 엔진 (MobileSAM 통합)

    RGB → Canny Edge 변환 후 EdgeAuxResNet18로 추론한다.
    Depth는 Aux 피처(물리 치수) 계산에만 사용, 모델 입력은 Edge만.
    """
    ...
    def __init__(self, model_path: str, class_names: list,
        ...
        self.model = EdgeAuxResNet18(len(class_names), pretrained=False)
        ...
        self.transform = EdgeTransform(IMAGE_SIZE, is_train=False)

        if self.device.type == "cuda":
            print("CUDA 워밍업 중...")
            dummy_img = torch.randn(1, EDGE_IN_CHANNELS, IMAGE_SIZE, IMAGE_SIZE,
                                    device=self.device)
    ...
    def _log_timing(self, t_start, t_preprocess, t_sam, t_model):
        ...
        print(f"[추론 #{self._log_count}] "
              f"전처리+Edge: {pre_ms:.0f}ms | SAM: {sam_ms:.0f}ms | "
...
engine: PyTorchEdgeInferenceEngine = None  # type: ignore[assignment]
...
if __name__ == "__main__":
    ...
    engine = PyTorchEdgeInferenceEngine(
        MODEL_PATH, class_names, use_sam=not args.no_sam)
    sam_status = "MobileSAM" if engine.sam_predictor else "depth 폴백"
    print(f"Edge PyTorch 엔진 준비 완료 "
          f"(디바이스: {engine.device}, 입력: {EDGE_IN_CHANNELS}ch Edge, "
