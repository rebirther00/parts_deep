"""굴착기 Door 실시간 분류 추론 서버 (RGBE 버전)

ZED X Mini 카메라 + RGBE(RGB+Edge) PyTorch 모델을 사용하여
도어를 실시간 분류하고 결과를 웹에 표시한다.
RGB texture + Canny Edge shape 정보를 동시에 활용.

실행:
    python 04_door_realtime_inference_rgbe.py
    python 04_door_realtime_inference_rgbe.py --model artifacts/best_door_rgbe_model_5090.pth
...
from depth_utils import (
    RGBDAuxResNet18, MAX_DEPTH_MM, IN_CHANNELS,
...
parser.add_argument(
    "--model", type=str,
    default=os.path.join(ARTIFACTS_DIR, "best_door_rgbe_model_5090.pth"),
    help="모델 파일 경로 (기본: artifacts/best_door_rgbe_model_5090.pth)",
)
parser.add_argument(
...
class PyTorchRGBEInferenceEngine:
    """PyTorch RGBE + 보조 피처 추론 엔진 (MobileSAM 통합)

    RGB + Canny Edge 4채널 입력. Depth는 Aux 피처 계산에만 사용.
    """
    ...
    def __init__(self, model_path: str, class_names: list,
        ...
        if self.device.type == "cuda":
            print("CUDA 워밍업 중...")
            dummy_img = torch.randn(1, RGBE_IN_CHANNELS, IMAGE_SIZE, IMAGE_SIZE,
                                    device=self.device)
    ...
    def infer(self, frame: np.ndarray, depth: np.ndarray = None) -> tuple:
        ...
        # RGBE: Transform이 RGB만 받아서 내부에서 Edge 생성
...
engine: PyTorchRGBEInferenceEngine = None  # type: ignore[assignment]
...
if __name__ == "__main__":
    ...
    engine = PyTorchRGBEInferenceEngine(
        MODEL_PATH, class_names, use_sam=not args.no_sam)
    sam_status = "MobileSAM" if engine.sam_predictor else "depth 폴백"
    print(f"RGBE PyTorch 엔진 준비 완료 "
          f"(디바이스: {engine.device}, 입력: {RGBE_IN_CHANNELS}ch RGBE, "
          f"전경분리: {sam_status})")
