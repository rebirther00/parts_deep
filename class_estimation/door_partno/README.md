...
- 블라인드 35세션 663장(학습 미사용, 35세션 전부 확정): 규칙 100%(판정률 99.8%) · 35세션을 뺀 4채널 검출기 100%(판정률 97.1%) · CNN multi448 91.0% / part14 90.3% / multi640 95.2% — CNN 오류는 전부 형상군 혼동(레이더 오류 0).
...
- **CNN 형상군 오류 원인 확인 실험(학습률 1e-4, 2026-09-23 저녁)**: 블라인드 663장 — multi448 lr1e-4 **99.8%**(2에폭 체크포인트, seed916 에서 거의 안 움직인 상태) / part14 lr1e-4 91.0%(37에폭) / multi448 lr1e-4b 91.3%(40에폭, val 손실 동점 선택). 세 run 의 오류는 전부 9/22 E38_LH_FRT 3세션 → E30_LH_FRT.
  → 레이더 헤드 탓이 아니고(헤드 없는 14클래스도 동일) 학습률 자체도 아님. **학습 세션에 오래 맞출수록 E30/E38 FRT(폭 차 47mm) 경계가 학습 세션 외관에 과적합**되어 새 세션에서 무너짐(seed916 도 같은 3세션에서 3장 오답). CNN 의 구조적 한계로 결론.
