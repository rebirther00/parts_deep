...
| `templates/factory.html` | 터치 UI (9버튼 + 시작/조기종료 + 시스템 종료 + 상태/프리뷰) |
| `deploy/door-capture.service` | systemd 유닛 (부팅 자동시작, 죽으면 5초 후 재시작) |
| `deploy/kiosk.sh` + `door-kiosk.desktop` | GUI 로그인 시 Firefox 키오스크 자동 실행 |
| `deploy/install_factory.sh` | 설치 스크립트 (`sudo bash deploy/install_factory.sh` 1회) |

## 동작 사양

- 취득: 시작 후 **2초 워밍업**(노출 안정화, 저장 안 함) → **2초에 1장** rgb/depth 쌍 저장 → **60초 후 자동 종료**. 조기 종료 버튼 있음.
...
3. 1분 후 자동 종료됨 (버튼 누를 필요 없음). 다음 도어부터 2번 반복
