...
const s1 = [
  h1('1. 요약'),
  p('굴착기 도어 8종은 형상이 거의 같고 치수(폭 830~1458mm)만 다르다. 형상을 학습하는 CNN 분류기는 사무실 데이터에서 100%였으나 현장에서 14%로 무너졌다. 본 보고서는 이를 대체하는 새 방식 — 도어 모서리 프레임 홀 2개의 거리 D를 재서 CAD 치수와 대조하는 기하 판정 — 의 학습 기법과 평가 결과를 정리한다. AI(CNN)는 형상 분류가 아니라 "홀 위치 검출"에만 쓰고, 판정은 설명 가능한 치수 비교로 한다.'),
  table(['항목', '결과'], [
...
const s2 = [
  h1('2. 판별 원리'),
  h2('2.1 왜 홀 거리인가'),
  bullet('8종 모두 좌우 세로 프레임의 래치 각공 쪽 끝에 Ø6mm 홀이 한 쌍 있고, 위치가 가장자리에서 52~54mm·그 변에서 105mm로 동일하다 (CAD 8종 분석). 따라서 두 홀의 거리 D = 도어 폭 − 106mm. (이 변은 CAD 렌더에서는 위쪽이지만 실제 장착 기준으로는 바닥 쪽이다. 세로 프레임의 반대쪽 끝에는 홀이 없다.)'),
  ...
  ...img('corner_marked_E25_door_RH.png', 300, '그림 1. CAD 내판 렌더 — 판별에 쓰는 모서리 홀 2개 (E25_RH, D=886mm)'),
  ...
  h2('2.2 판정 파이프라인'),
  num('CNN이 원본 프레임에서 6점을 검출: 래치 볼트홀 4개(각공 주변) + 모서리 홀 2개(힌지측·래치측).'),
  ...
  num('통과 시 CAD D 표에서 최근접 클래스. 실시간에서는 10프레임 윈도의 판정 프레임 D 중앙값으로 집계한다. 속성 파이프라인(통풍구 U-Net)의 그룹 판별은 보조 정보로: 홀 D의 최근접 마진이 30mm 이상이면 홀 판정 우선, 미만이면 U-Net 그룹 내 최근접, 15mm 미만이면 "보류". 판정 프레임이 3개 미만이면 속성 파이프라인으로 폴백한다.'),
...
const s3 = [
  ...
  bullet('전용 브라우저 도구(15_label_holes.py)로 클릭 라벨링: 6배 확대경, 점 순서 자동 진행, 자동 저장. 134장 × 6점을 약 1시간에 완료.'),
...
const s4 = [
  ...
  p('오판 8건(0.7%)은 검출 오류가 아니라 거리 측정 오차다. 홀은 정확히 잡혔으나 depth 평면 기반 D가 CAD보다 20~58mm 벗어나(E30_E38_RH 4건: −58~+38mm, E30·E38 FRT 4건: −21~−27mm) 이웃 클래스로 넘어갔다. 판정된 814장의 측정 오차는 클래스별 |오차| p95가 11~24mm로, 클래스 간격 41mm(FRT)·71mm(E30_E38_RH↔E30_LH_RR)에 대해 여유가 작은 구간에서 오판이 난다. 원인은 근사 intrinsics(fx=1065 가정, cx·cy 중심 가정)와 depth 평면 피팅 영역의 편차로, 카메라 정식 캘리브레이션 값을 쓰면 줄어든다.'),
  p('대응으로 속성 파이프라인(통풍구 U-Net)의 그룹 판별을 결합했다. 정답 그룹을 제약으로 걸면(그룹 내 최근접) 814장 정확도가 99.0→99.5%로 오르며 RH↔RR 혼동 4건이 해소된다. 다만 U-Net 그룹 자체가 틀리는 경우가 있어(현장 E25_RH 세션에서 U-Net은 RR로 판정) 그룹을 하드 제약으로 두지 않고, 홀 D의 최근접 마진이 30mm 미만일 때만 그룹 제약을 적용하고 그 아래(15mm)면 보류한다. 마진이 충분하면 홀 판정을 우선한다(현장 세션: 마진 74mm → E25_RH 정답).'),
  h2('4.5 보류 샘플'),
...
const s5 = [
  h1('5. 사용법'),
  p('실행 순서는 기존 A/B 파이프라인과 같은 번호 규칙을 따른다: ⑮ 라벨링 → ⑯ 학습 → ⑰ 평가 → ⑭ 실시간 추론(하이브리드). 모든 스크립트는 door_pipeline 디렉터리에서 실행하며, scripts/ 래퍼가 venv 활성화와 경로를 처리한다.'),
  table(['단계', '명령', '입력 → 출력'], [
    ['라벨링 ⑮', 'scripts/hole_label.sh [장수] [추가폴더]  (= python 15_label_holes.py)', '브라우저 :8090 → labels/holes/*.json'],
    ['학습 ⑯', 'scripts/hole_train.sh [epochs=80]  (= python 16_train_hole_landmarks.py)', 'labels/holes → attribute_models/hole_landmarks/model.pth, DB training_sessions'],
    ['평가 ⑰', 'scripts/hole_evaluate.sh [폴더]  (= python 17_evaluate_hole_classifier.py)', 'test 분할·datasets·현장 → eval_classifier.json, report/hole_analysis/samples/*, DB evaluation_results'],
    ['단일 이미지', 'python scripts/hole_classify_image.py rgb.png [--out r.jpg]', '판정 JSON + 표시 이미지'],
    ['실시간 ⑭', 'scripts/hole_inference.sh [리플레이폴더]  (= python 14_realtime_inference_attribute.py)', 'ZED 또는 폴더 재생 → 웹 UI :5003, /api/inference_result'],
  ], [2, 5.5, 5.5]),
  gap(),
  code([
    '# 예: 현장 세션 리플레이로 확인',
  ...
  table(['필드', '의미'], [
    ...
    ['source', 'hole(홀 판별기) / attr(속성 파이프라인 폴백) / conflict(홀 마진 부족 + 그룹 불일치 → 보류)'],
    ['hole.policy / margin_mm / attr_group', 'hole(홀 단독) · hole_over_attr(그룹 불일치지만 마진 충분) · group_constrained(그룹 내 최근접), 최근접 마진, U-Net 그룹'],
...
const s6 = [
  ...
  table(['순위', '항목', '내용', '기대 효과'], [
    ['1', '거리 측정 정밀화', 'ZED SDK의 정식 intrinsics(fx·fy·cx·cy)로 depth 평면 거리 계산, 볼트 피치 스케일과 교차검증. 현장 카메라별 K 재캘리브레이션', '측정 오차 p95 20mm→10mm 이하, FRT 간 오판 제거'],
    ...
    ['5', 'U-Net 그룹 판별 점검', '현장 E25_RH 세션에서 U-Net이 RR로 오판. 현장 데이터로 U-Net 그룹 정확도 재평가 후 마진 임계(30mm) 조정', '하이브리드 판정 신뢰도'],
  ...
  table(['경로', '내용'], [
    ...
    ['16_train_hole_landmarks.py', '학습 + 홀드아웃 평가 (DB 기록)'],
    ['17_evaluate_hole_classifier.py', '판정 평가 (test 분할·datasets·현장·임의 폴더)'],
    ['14_realtime_inference_attribute.py', '실시간 서버 — 홀 판별기 1순위, 속성 파이프라인 폴백'],
    ['15_label_holes.py · tools/make_hole_samples.py', '라벨링 도구 · 샘플/학습곡선 생성'],
