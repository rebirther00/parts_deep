...
Tremblay et al.[16]은 도메인 랜덤화를 객체 검출에 확장 적용하여, 비현실적 합성 데이터만으로 학습한 모델이 실세계 객체 검출에서 경쟁력 있는 성능을 달성함을 보고하였다.

이러한 도메인 랜덤화 기법은 본 연구의 텍스처 변형 증강과 개념적으로 유사하다. 본 연구에서 사용한 오프라인 증강(Brightness, Contrast, Noise, Blur 변형)은 시뮬레이터 없이 실물 데이터에 직접 적용하는 경량화된 도메인 랜덤화로 볼 수 있으며, RGBE 하이브리드 입력 표현은 이러한 도메인 갭에 대한 구조적 강건성을 제공한다는 점에서도 도메인 랜덤화와 상호 보완적이다.
...
> **Table 8.** Texture variation robustness comparison (accuracy drop from original, mean ± std, 448×448)
...
Table 9는 448×448 해상도에서 모델 쌍별 Macro F1 기반 paired t-test 결과를 보여준다.

> **Table 9.** Pairwise paired t-test results (Macro F1, 448×448)
...
> **Table 10.** Training convergence comparison (448×448, mean ± std, 5-seed)
...
> **Table 11.** Resolution comparison — overall accuracy (mean ± std, 5-seed)
...
> **Table 12.** Resolution comparison — robustness (accuracy drop, mean ± std)

| Model | Resolution | Δ(Aug FG) | Δ(Aug Full) | Avg Δ | Rank |
|-------|:--------:|:---------:|:----------:|:-----:|:----:|
| **RGBE Hybrid** | **448** | **−8.36 ± 1.65** | **−6.14 ± 1.70** | **−7.25** | **1** |
| Baseline RGBD | 448 | −9.21 ± 1.57 | −7.09 ± 1.89 | −8.15 | 2 |
| Texture Aug RGBD | 448 | −9.74 ± 2.81 | −7.62 ± 4.04 | −8.68 | 3 |
| **Texture Aug RGBD** | **224** | **−7.62 ± 3.10** | **−4.44 ± 1.33** | **−6.03** | **1** |
| Baseline RGBD | 224 | −8.36 ± 1.69 | −8.25 ± 2.84 | −8.30 | 2 |
| RGBE Hybrid | 224 | −14.18 ± 4.26 | −12.38 ± 2.63 | −13.28 | 3 |
| Edge-only | 448 | −29.52 ± 5.49 | −25.08 ± 2.96 | −27.30 | 4 |
| Edge-only | 224 | −20.42 ± 2.07 | −17.88 ± 2.07 | −19.15 | 4 |
...
다만, RGBE, Baseline RGBD, Texture Aug RGBD 3개 모델 간의 차이는 통계적으로 유의하지 않았다(모든 p > 0.08). 5-seed 반복 실험의 한계로 인해 검정력(statistical power)이 충분하지 않을 수 있으며, RGBE Hybrid의 하락폭 표준편차가 가장 작은 점(±1.65 vs ±2.81)은 RGBE가 보다 안정적인 강건성을 제공할 가능성을 시사한다.
...
(4) RGBE Hybrid는 448 해상도에서 가장 작은 하락폭 표준편차(±1.65)를 보여, seed 간 안정성 측면에서 가장 일관된 강건성을 제공하였다.

다만, 본 연구는 동일 개체 기반 데이터에서의 오프라인 증강 조건에 한정된 비교이므로, 결과의 일반화에는 주의가 필요하다. 향후 연구로는 다른 개체의 도어를 추가 촬영한 인스턴스 간 일반화 검증, 실제 현장 환경에서의 실증, RGBE + Texture Aug 결합 전략 탐색, 그리고 NVIDIA Jetson Orin 환경에서의 실시간 추론 최적화를 계획하고 있다.
...
[15] J. Tremblay, A. Prakash, D. Acuna, M. Brophy, V. Jampani, C. Anil, T. To, E. Cameracci, S. Boochoon, and S. Birchfield, "Training deep networks with synthetic data: Bridging the reality gap by domain randomization," in Proc. IEEE/CVF Conf. Computer Vision and Pattern Recognition Workshops (CVPRW), 2018.

[16] F. Sadeghi and S. Levine, "(CAD)²RL: Real single-image flight without a single real image," in Proc. Robotics: Science and Systems (RSS), 2017.
...
<!-- Fig. 5. 해상도별 강건성 비교 (resolution_comparison.png)        -->
<!--                                                               -->
<!-- [선택적 그림]                                                  -->
<!-- Fig. 6. 강건성 비교 막대 그래프 (robustness_comparison_448.png) -->
<!-- Fig. 7. 입력 표현별 전처리 결과 비교 (동일 원본에서 시각화)     -->
<!-- Fig. 8. 4종 모델 혼동 행렬 (각 모델별 대표 seed)               -->
<!-- ============================================================ -->
