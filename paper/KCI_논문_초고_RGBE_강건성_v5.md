...
#### 5.9.1 4종 모델 attention 비교

Fig. 6에 448×448 해상도에서 3개 대표 클래스(E25_LH_FRT, E30_E38_RH, E38_LH_RR)에 대한 4종 모델의 Grad-CAM heatmap을 제시한다.

<!-- [Fig. 6 삽입: class_estimation/door_paper/artifacts/summary_noaux/gradcam_fig6.png] -->

> **Fig. 6.** Grad-CAM comparison of four models on three representative classes (448×448, seed=42). Warmer colors indicate higher attention.

4종 모델의 attention 패턴에서 다음과 같은 차이가 관찰되었다.

첫째, **RGBE Hybrid는 도어 윤곽선 및 구조적 특징(경첩, 구멍, 모서리)에 집중하는 경향**을 보였다. 이는 Edge 채널이 제공하는 명시적 형상 정보가 모델의 attention을 구조적 영역으로 유도함을 시사한다.

둘째, **Baseline RGBD와 Texture Aug RGBD는 도어 표면 전체에 걸쳐 비교적 넓은 attention 분포**를 보였다. 이는 RGB 텍스처 정보에 대한 의존도가 상대적으로 높음을 반영한다.
...
> **Fig. 7.** Grad-CAM robustness comparison between Baseline RGBD and RGBE Hybrid under texture variation (Original vs Augmented, 448×448, seed=42).
...
둘째, **Baseline RGBD는 텍스처 변형 시 attention 영역이 이동하거나 분산**되는 경향이 관찰되었다. 이는 RGB 텍스처 정보의 변화가 모델의 판단 근거를 불안정하게 만드는 메커니즘을 시각적으로 확인시킨다.

이러한 Grad-CAM 분석 결과는 5.4절의 강건성 지표(RGBE Avg Δ = −7.25%p vs RGBD −8.15%p)와 일관되며, RGBE Hybrid의 강건성이 Edge 채널에 의한 구조적 attention 안정화에서 기인함을 뒷받침한다.
...
따라서 본 연구의 주요 기여는 RGBE의 통계적 우위 입증이 아닌, (1) Edge 채널의 보조적 활용이 강건성에 긍정적 경향을 보인다는 관찰, (2) 해상도에 따라 최적 입력 표현이 달라진다는 실무적 발견, (3) Edge-only 접근의 명확한 한계 확인, (4) Depth 대비 Edge의 조건부 우위, (5) Grad-CAM을 통한 Edge 채널의 구조적 attention 안정화 메커니즘의 시각적 확인에 있다.
...
구체적으로, Canny Edge는 물체의 주요 윤곽선, 구멍, 경첩 등의 형상 특징을 명시적으로 부호화하며, 이러한 구조적 특징은 Brightness나 Contrast 변화에 비교적 견고하다. 반면 Depth 채널은 카메라-물체 거리에 기반한 전역적 기하 정보를 제공하지만, 모델이 텍스처 변형을 보상하는 데 직접적인 단서가 되기 어렵다. 5.9절의 Grad-CAM 분석은 이러한 해석을 시각적으로 뒷받침한다. RGBE Hybrid가 도어 윤곽 및 구조적 특징 영역에 attention을 집중하고, 텍스처 변형 후에도 해당 패턴이 안정적으로 유지되는 반면, Baseline RGBD는 텍스처 변형 시 attention이 이동·분산되는 것이 관찰되었다(Fig. 6, 7).
...
(6) Grad-CAM 분석을 통해, RGBE Hybrid가 도어 윤곽 및 구조적 특징 영역에 attention을 집중하며, 텍스처 변형 후에도 해당 패턴이 안정적으로 유지됨을 시각적으로 확인하였다. 이는 Edge 채널이 모델의 attention을 구조적 영역으로 유도하여 텍스처 변형 강건성에 기여하는 메커니즘에 대한 증거를 제공한다.
...
<!--                                                               -->
<!-- [v5에서 추가됨 - Grad-CAM 시각적 해석]                         -->
<!-- - 5.9절 + Fig. 6, 7로 Grad-CAM 분석 완료                      -->
<!-- - gradcam_analysis.py 스크립트 작성 완료                       -->
<!-- - 참고문헌 [18] Selvaraju et al. (Grad-CAM) 추가              -->
<!--                                                               -->
<!-- [DOCX 변환 시 주의사항]                                        -->
<!-- - 참고문헌 번호: v5에서 [18]이 추가됨 (Grad-CAM)             -->
