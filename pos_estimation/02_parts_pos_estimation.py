...
class PartsXYDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        with open(s.pose_path, "r") as f:
            pose = json.load(f)
        # 작업대(world) 기준 xy 사용
        t_world = pose["raw_pose_world"]["t_xyz_m"]
        xy = np.asarray([float(t_world[0]), float(t_world[1])], dtype=np.float32)
...
def _compute_xy_stats(train_samples: list[Sample]):
    """train split에서 world xy의 mean/std 계산"""
    xs = []
    for s in train_samples:
        with open(s.pose_path, "r") as f:
            pose = json.load(f)
        t_world = pose["raw_pose_world"]["t_xyz_m"]
        xs.append([float(t_world[0]), float(t_world[1])])
