#!/usr/bin/env python3
# ==========================================
# Depth 기반 6DoF Pose Estimation (RTX 5090 최적화)
...
# - 배치 사이즈 증가 (16 → 64)
# - 에포크 수 증가 (100 → 150)
# - Early Stopping patience 증가 (20 → 40)
# - DataLoader 멀티프로세싱 최적화
# - non_blocking 데이터 전송
#
# 핵심 아이디어:
...
# RTX 5090 최적화 학습 설정
BATCH_SIZE = 64  # 16 → 64 (RTX 5090 32GB VRAM 활용)
NUM_EPOCHS = 150  # 100 → 150 (큰 배치에 맞게 증가)
LEARNING_RATE = 1e-4
WEIGHT_DECAY = 1e-4
TRAIN_RATIO = 0.8
...
# Depth 기반 GT 설정
DEPTH_MIN = 0.01  # 최소 유효 Depth (m)
DEPTH_MAX = 100.0 # 최대 유효 Depth (m)
FOREGROUND_PERCENTILE = 10  # 전경 결정 백분위수

# Early Stopping 설정 (RTX 5090 최적화)
EARLY_STOP_PATIENCE = 40  # 20 → 40 (큰 배치에 맞게 증가)
...
def train_model(dataset_dir=DATASET_DIR, use_resnet50=False, use_bbox_crop=False):
    ...
    print(f"\n{'='*70}")
    print("Depth 기반 GT로 6DoF Pose Estimation 학습 (RTX 5090 최적화)")
    print(f"{'='*70}")
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {epoch+1:03d} Train", leave=False):
            # RTX 5090 최적화: non_blocking 전송
            rgb = batch['rgb'].to(device, non_blocking=True)
            depth = batch['depth'].to(device, non_blocking=True)
            position = batch['position'].to(device, non_blocking=True)
            rotation_6d = batch['rotation_6d'].to(device, non_blocking=True)
        ...
        with torch.no_grad():
            for batch in test_loader:
                ...
                rotation_6d_gt = batch['rotation_6d'].to(device, non_blocking=True)
                class_idx = batch['class_idx'].to(device, non_blocking=True)
        ...
        if avg_pos_error < best_pos_error:
            ...
            }, os.path.join(ARTIFACTS_DIR, 'depth_gt_6dof_best_5090.pt'))
        else:
            patience_counter += 1
    ...
    print(f"\n{'='*70}")
    print(f"🎯 6DoF 학습 완료! (RTX 5090 최적화)")
    print(f"  최고 위치 오차: {best_pos_error:.2f}mm")
    print(f"  최고 회전 오차: {best_rot_error:.2f}°")
    print(f"모델 저장: {os.path.join(ARTIFACTS_DIR, 'depth_gt_6dof_best_5090.pt')}")
    ...
    for class_dir in class_dirs:
        ...
        for depth_file in depth_files:
            ...
            if os.path.exists(pose_file):
                ...
                print(f"    차이: {diff*1000:.1f}mm {'✅' if diff < 0.5 else '❌'}")
...
def evaluate_model(dataset_dir=DATASET_DIR, use_resnet50=False, use_bbox_crop=False):
    """저장된 모델로 Train/Test 데이터 평가 (RTX 5090 버전)"""
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Device: {device}")
    ...
    print(f"\n{'='*70}")
    print("📊 Train/Test 오차 통계 (RTX 5090 버전)")
    print(f"{'='*70}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Depth 기반 6DoF Pose Estimation (RTX 5090 최적화)")
    ...
    LOG_PATH = setup_logging(f"07_pose_5090_{args.mode}")
