#!/usr/bin/env python3
# ==========================================
# Depth 기반 6DoF Pose Estimation V2 (RTX 5090 최적화)
...
#
# 사용법:
#   python 09_depth_based_pose_v2_5090.py --mode train
#   python 09_depth_based_pose_v2_5090.py --mode train --bbox_crop
#
# ==========================================
...
# 로그 설정
LOG_PATH = setup_logging("09_depth_pose_v2_5090")

DATASET_DIR = os.path.join(PROJECT_DIR, "dataset_pos_depth")
ARTIFACTS_DIR = os.path.join(PROJECT_DIR, "artifacts")

# RTX 5090 최적화 학습 설정
BATCH_SIZE = 64  # RTX 5090 32GB VRAM 활용
NUM_EPOCHS = 150
LEARNING_RATE = 1e-4
WEIGHT_DECAY = 1e-4
TRAIN_RATIO = 0.8
...
def train_model(dataset_dir=DATASET_DIR, force_cpu=FORCE_CPU, use_bbox_crop=USE_BBOX_CROP):
    ...
    print("\n" + "=" * 80)
    print("📦 V2 모델 학습 (EfficientNetV2-S + RandAugment) - RTX 5090 최적화")
    print("=" * 80)
    ...
    print(f"⚙️  DataLoader num_workers: {NUM_WORKERS}, prefetch_factor: {PREFETCH_FACTOR}")
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if avg_pos_error < best_pos_error:
            ...
            torch.save({
                ...
                'model_version': 'V2_EfficientNetV2S_5090',
                'use_bbox_crop': use_bbox_crop
            }, os.path.join(ARTIFACTS_DIR, 'depth_gt_pose_v2_5090_best.pt'))
        else:
            patience_counter += 1
    ...
    print(f"\n{'='*70}")
    print(f"🎉 V2 학습 완료! (RTX 5090 최적화)")
    print(f"   최고 위치 오차: {best_pos_error:.2f}mm")
    if use_rotation:
        print(f"   최고 자세 오차: {best_rot_error:.2f}°")
    print(f"   모델 저장: {os.path.join(ARTIFACTS_DIR, 'depth_gt_pose_v2_5090_best.pt')}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Depth 기반 6DoF Pose Estimation V2 (RTX 5090 최적화)")
