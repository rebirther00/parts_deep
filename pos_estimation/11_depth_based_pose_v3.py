#!/usr/bin/env python3
# ==========================================
# Depth 기반 6DoF Pose Estimation V3
# ConvNeXt-Small 백본 + 개선된 학습 전략
# ==========================================
#
# V2 대비 개선사항:
# 1. EfficientNetV2-S → ConvNeXt-Small (최신 CNN 아키텍처)
# 2. GELU 활성화 함수 사용
# 3. Stochastic Depth (Drop Path) 적용
#
# 사용법:
#   python 10_depth_based_pose_v3.py --mode train
#   python 10_depth_based_pose_v3.py --mode train --bbox_crop
#
# ==========================================
...
# 로그 설정
LOG_PATH = setup_logging("11_depth_pose_v3")
...
BATCH_SIZE = None
NUM_EPOCHS = 150
LEARNING_RATE = 5e-5  # ConvNeXt는 더 작은 LR 선호
WEIGHT_DECAY = 1e-4
TRAIN_RATIO = 0.8
FORCE_CPU = False
USE_BBOX_CROP = False
...
class RGBDepthTo3DModelV3(nn.Module):
    ...
    def __init__(self, num_classes=4, depth_features=256, use_rotation=True):
        ...
        print(f"  모델 V3: ConvNeXt-Small + DepthEncoder → 3D Position {rotation_str}")
...
def train_model(dataset_dir=DATASET_DIR, force_cpu=FORCE_CPU, use_bbox_crop=USE_BBOX_CROP):
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if avg_pos_error < best_pos_error:
            ...
            torch.save({
                ...
                'model_version': 'V3_ConvNeXt_Small',
                'use_bbox_crop': use_bbox_crop
            }, os.path.join(ARTIFACTS_DIR, 'depth_gt_pose_v3_best.pt'))
        else:
            patience_counter += 1
    ...
    print(f"   모델 저장: {os.path.join(ARTIFACTS_DIR, 'depth_gt_pose_v3_best.pt')}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Depth 기반 6DoF Pose Estimation V3 (ConvNeXt-Small)")
