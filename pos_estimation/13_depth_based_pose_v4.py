#!/usr/bin/env python3
# ==========================================
# Depth 기반 6DoF Pose Estimation V4
# Swin Transformer V2-B + 1024×1024 고해상도
# ==========================================
#
# V3 대비 개선사항:
# 1. ConvNeXt-Small → Swin Transformer V2-B (고해상도 특화)
# 2. 입력 해상도: 224×224 → 1024×1024 (원본 해상도 유지)
# 3. 윈도우 기반 Self-Attention으로 위치 정보 보존
# 4. GB10 (128GB) GPU 최적화
#
# 사용법:
#   python 11_depth_based_pose_v4.py --mode train
#   python 11_depth_based_pose_v4.py --mode train --bbox_crop
#
# ==========================================
...
FORCE_CPU = False
USE_BBOX_CROP = False
WARMUP_EPOCHS = 10    # Transformer는 더 긴 warmup 필요

# 입력 해상도 설정 (V4: 1024×1024 원본 유지)
INPUT_SIZE = 1024
...
def get_optimal_batch_size(data_size, force_cpu=False):
    """GPU 메모리에 따라 최적 배치 사이즈 결정 (1024 해상도 기준)"""
    # 1024 해상도는 메모리를 많이 사용하므로 보수적으로 설정
...
# ==========================================
# 데이터셋 V4: 1024×1024 고해상도
# ==========================================
class DepthGTDatasetV4(Dataset):
    """V4: 1024×1024 고해상도 데이터셋 (Swin V2 최적화)"""
    ...
    def __init__(self, dataset_dir, split='train', train_ratio=TRAIN_RATIO, 
                 ...
                 lazy_loading=False, input_size=INPUT_SIZE):
        ...
        # V4: 1024×1024 고해상도 데이터 증강
        if self.use_augmentation:
            self.rgb_transform = transforms.Compose([
                transforms.Resize((self.input_size, self.input_size)),
                transforms.ColorJitter(brightness=0.2, contrast=0.2, saturation=0.1, hue=0.05),
                transforms.RandomHorizontalFlip(p=0.3),
                # 고해상도에서는 회전을 약하게 (위치 정보 보존)
...
class DepthEncoderV4(nn.Module):
    ...
    def __init__(self, out_features=256, input_size=1024):
        ...
        # 1024 → 512 → 256 → 128 → 64 → 32 → 16 → 8 (8단계)
...
# ==========================================
# 모델 V4: Swin Transformer V2-B + Depth → 6DoF Pose
# ==========================================
class RGBDepthTo3DModelV4(nn.Module):
    """V4: Swin Transformer V2-B + Depth 융합 → 6DoF Pose 예측
    
    핵심 특징:
    - Swin V2-B: 윈도우 기반 Self-Attention (고해상도 효율적)
    - 1024×1024 입력으로 위치 정보 최대한 보존
    - 상대적 위치 인코딩으로 해상도 변화에 강건
    """
    ...
    def __init__(self, num_classes=4, depth_features=256, use_rotation=True, input_size=1024):
        ...
        print(f"  모델 V4: Swin Transformer V2-B + DepthEncoder → 3D Position {rotation_str}")
        print(f"  입력 해상도: {input_size}×{input_size} (고해상도)")
...
def train_model(dataset_dir=DATASET_DIR, force_cpu=FORCE_CPU, use_bbox_crop=USE_BBOX_CROP):
    ...
    # 데이터셋 로드
    print("\n" + "=" * 80)
    print("📦 V4 모델 학습 (Swin Transformer V2-B + 1024×1024 고해상도)")
    print("=" * 80)
    
    if use_bbox_crop:
        print("📦 bbox_2d ROI crop 모드 활성화")
    
    train_dataset = DepthGTDatasetV4(
    ...
    if BATCH_SIZE is None:
        ...
        print(f"📦 배치 사이즈 자동 설정: {batch_size} (1024×1024 해상도)")
    else:
    ...
    rotation_criterion = nn.SmoothL1Loss()
    ...
    # Warmup + Cosine Annealing (Transformer는 더 긴 warmup)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if avg_pos_error < best_pos_error:
            ...
            torch.save({
                ...
                'use_rotation': use_rotation,
                'model_version': 'V4_SwinV2B_1024',
                'input_size': INPUT_SIZE,
                'use_bbox_crop': use_bbox_crop
            }, os.path.join(ARTIFACTS_DIR, 'depth_gt_pose_v4_best.pt'))
        else:
            patience_counter += 1
    ...
    print(f"\n{'='*70}")
    print(f"🎉 V4 학습 완료! (Swin V2-B + 1024×1024)")
    ...
    print(f"   모델 저장: {os.path.join(ARTIFACTS_DIR, 'depth_gt_pose_v4_best.pt')}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Depth 기반 6DoF Pose Estimation V4 (Swin V2-B + 1024×1024)")
