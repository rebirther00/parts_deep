#!/usr/bin/env python3
# ==========================================
# Depth 기반 6DoF Pose Estimation V4 (RTX 5090 최적화)
# Swin Transformer V2-B + 고해상도 1024×1024
# ==========================================
#
# RTX 5090 최적화 (32GB VRAM):
# - 1024×1024 고해상도 입력
# - 배치 사이즈 자동 조정 (VRAM 기반)
# - AMP + Gradient Checkpointing
#
# V3 대비 개선사항:
# 1. ConvNeXt-Small → Swin Transformer V2-B (더 강력한 아키텍처)
# 2. 입력 해상도 224 → 1024 (고해상도)
# 3. Window-based Self-Attention (효율적인 고해상도 처리)
#
# 사용법:
#   python 13_depth_based_pose_v4_5090.py --mode train
#
# ==========================================
...
LOG_PATH = setup_logging("13_depth_pose_v4_5090")

DATASET_DIR = os.path.join(PROJECT_DIR, "dataset_pos_depth")
ARTIFACTS_DIR = os.path.join(PROJECT_DIR, "artifacts")

# RTX 5090 최적화 학습 설정 (고해상도)
NUM_EPOCHS = 150
LEARNING_RATE = 1e-4  # Swin V2 권장 LR
WEIGHT_DECAY = 5e-2   # Swin V2 권장 Weight Decay
TRAIN_RATIO = 0.8
WARMUP_EPOCHS = 10    # Swin V2는 더 긴 Warmup 권장
...
# V4 핵심: 고해상도 입력
INPUT_SIZE = 1024  # 1024×1024 고해상도
...
# ==========================================
# RTX 5090 최적화: 배치 사이즈 자동 조정
# ==========================================
def get_optimal_batch_size():
    """GPU VRAM에 따라 최적 배치 사이즈 결정 (1024 해상도용)"""
    if not torch.cuda.is_available():
        return 2
    
    total_mem_gb = torch.cuda.get_device_properties(0).total_memory / (1024**3)
    
    # 1024×1024 해상도 기준 배치 사이즈
    if total_mem_gb >= 80:  # GB200+ 급
        return 16
    elif total_mem_gb >= 32:  # RTX 5090 (32GB)
        return 8
    elif total_mem_gb >= 24:  # RTX 4090 (24GB)
        return 4
...
# 데이터셋 V4 (고해상도 1024)
# ==========================================
class DepthGTDatasetV4(Dataset):
    def __init__(self, dataset_dir, split='train', train_ratio=TRAIN_RATIO, 
        ...
        if self.use_augmentation:
            self.rgb_transform = transforms.Compose([
                ...
                transforms.RandomRotation(degrees=5),  # 고해상도에서는 작은 회전
                transforms.ToTensor(),
        ...
        print(f"  ⚡ V4 고해상도: {INPUT_SIZE}×{INPUT_SIZE}")
...
# ==========================================
# Depth Encoder V4 (1024 최적화)
# ==========================================
class DepthEncoderV4(nn.Module):
    """V4: 1024×1024 입력용 Depth Encoder"""
    def __init__(self, out_features=256):
        super().__init__()
        # 1024 → 512 → 256 → 128 → 64 → 32 → 16 → 8
        self.conv = nn.Sequential(
            nn.Conv2d(1, 32, 7, stride=2, padding=3), nn.BatchNorm2d(32), nn.GELU(),   # 512
            nn.Conv2d(32, 64, 3, stride=2, padding=1), nn.BatchNorm2d(64), nn.GELU(),  # 256
            nn.Conv2d(64, 128, 3, stride=2, padding=1), nn.BatchNorm2d(128), nn.GELU(), # 128
            nn.Conv2d(128, 256, 3, stride=2, padding=1), nn.BatchNorm2d(256), nn.GELU(), # 64
            nn.Conv2d(256, 384, 3, stride=2, padding=1), nn.BatchNorm2d(384), nn.GELU(), # 32
            nn.Conv2d(384, 512, 3, stride=2, padding=1), nn.BatchNorm2d(512), nn.GELU(), # 16
            nn.Conv2d(512, 512, 3, stride=2, padding=1), nn.BatchNorm2d(512), nn.GELU(), # 8
...
# ==========================================
# 모델 V4: Swin Transformer V2-B + 1024 → 6DoF Pose
# ==========================================
class RGBDepthTo3DModelV4(nn.Module):
    """V4: Swin Transformer V2-B + 1024×1024 고해상도"""
    def __init__(self, num_classes=4, depth_features=256, use_rotation=True):
        ...
        print(f"  모델 V4: Swin Transformer V2-B + DepthEncoderV4 → 3D Position {rotation_str}")
...
def train_model(dataset_dir=DATASET_DIR, force_cpu=FORCE_CPU, use_bbox_crop=USE_BBOX_CROP):
    ...
    print("\n" + "=" * 80)
    print("📦 V4 모델 학습 (Swin Transformer V2-B + 1024×1024) - RTX 5090 최적화")
    print("=" * 80)
    print(f"  📐 입력 해상도: {INPUT_SIZE}×{INPUT_SIZE}")
    print(f"  📦 배치 사이즈: {batch_size} (자동 조정)")
    
    train_dataset = DepthGTDatasetV4(dataset_dir, split='train', use_augmentation=True, use_bbox_crop=use_bbox_crop)
    test_dataset = DepthGTDatasetV4(
    ...
    # 고해상도에서는 num_workers를 줄여 메모리 사용 최적화
    num_workers = min(NUM_WORKERS, 4) if INPUT_SIZE >= 1024 else NUM_WORKERS
    ...
    model = RGBDepthTo3DModelV4(num_classes=num_classes, depth_features=256, use_rotation=use_rotation).to(device)
    ...
    print(f"Epochs: {NUM_EPOCHS}, Warmup: {WARMUP_EPOCHS}, LR: {LEARNING_RATE}")
    print()
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if avg_pos_error < best_pos_error:
            ...
            torch.save({
                ...
                'use_rotation': use_rotation, 'model_version': 'V4_SwinV2B_1024_5090',
                'input_size': INPUT_SIZE, 'use_bbox_crop': use_bbox_crop
            }, os.path.join(ARTIFACTS_DIR, 'depth_gt_pose_v4_5090_best.pt'))
        else:
            patience_counter += 1
    ...
    print(f"   모델 저장: {os.path.join(ARTIFACTS_DIR, 'depth_gt_pose_v4_5090_best.pt')}")


if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Depth 기반 6DoF Pose V4 (RTX 5090)")
