...
데이터 누출(data leakage) 방지를 위해, 학습 시 사용된 **Test 분할(377장)**에 해당하는 이미지만 증강 데이터셋에서 추출하여 평가하였다. 즉, 학습에 사용된 Train 분할의 증강 버전은 평가에서 제외하여 공정한 비교를 보장한다.

**[증강 범위별 전체 정확도 비교]**

| 실험 조건 | 증강 범위 | 평가 이미지 수 | 전체 정확도 | Macro F1 |
|----------|----------|:-----------:|:---------:|:--------:|
| Test 분할 (datasets) | 없음 | 377장 | **100.00%** (377/377) | 100.00% |
| datasets_aug (Test 분할) | 전경만 (mask_only) | 377장 | **86.21%** (325/377) | 86.73% |
| datasets_aug2 (Test 분할) | 전체 이미지 | 377장 | **86.21%** (325/377) | 86.65% |

> **표 5-14.** 증강 범위별 정확도 비교 (Test 분할 기준, 데이터 누출 방지)

**[클래스별 상세 성능]**

| 클래스 | Test 분할 | datasets_aug<br/>(전경만) | datasets_aug2<br/>(전체) |
|--------|:--------:|:----------:|:----------:|
| E25_door_LH_FRT | 100.00% | 95.74% | 95.74% |
| E25_door_LH_RR | 100.00% | 63.83% | 65.96% |
| E25_door_RH | 100.00% | 80.85% | 89.36% |
| E30_E38_door_RH | 100.00% | 93.48% | 100.00% |
| E30_door_LH_FRT | 100.00% | 95.83% | 93.75% |
| E30_door_LH_RR | 100.00% | 97.92% | 93.75% |
| E38_door_LH_FRT | 100.00% | 86.96% | 84.78% |
| E38_door_LH_RR | 100.00% | 75.00% | 66.67% |

> **표 5-15.** 클래스별 증강 조건 비교 상세 (Test 분할 기준)
...
|--------|:---------:|:------:|:--------:|
| E25_door_LH_FRT | 95.74% | 95.74% | 95.74% |
| E25_door_LH_RR | 100.00% | 63.83% | 77.92% |
| E25_door_RH | 100.00% | 80.85% | 89.41% |
| E30_E38_door_RH | 84.31% | 93.48% | 88.66% |
| E30_door_LH_FRT | 86.79% | 95.83% | 91.09% |
| E30_door_LH_RR | 57.32% | 97.92% | 72.31% |
| E38_door_LH_FRT | 100.00% | 86.96% | 93.02% |
| E38_door_LH_RR | 100.00% | 75.00% | 85.71% |
| **평균 (Macro)** | **90.52%** | **86.20%** | **86.73%** |
...
| 클래스 | Precision | Recall | F1-Score |
|--------|:---------:|:------:|:--------:|
| E25_door_LH_FRT | 93.75% | 95.74% | 94.74% |
| E25_door_LH_RR | 100.00% | 65.96% | 79.49% |
| E25_door_RH | 100.00% | 89.36% | 94.38% |
| E30_E38_door_RH | 83.64% | 100.00% | 91.09% |
| E30_door_LH_FRT | 88.24% | 93.75% | 90.91% |
| E30_door_LH_RR | 56.96% | 93.75% | 70.87% |
| E38_door_LH_FRT | 100.00% | 84.78% | 91.76% |
| E38_door_LH_RR | 100.00% | 66.67% | 80.00% |
| **평균 (Macro)** | **90.32%** | **86.25%** | **86.65%** |
...
|------------|:----------:|:----------:|----------|
| E25_door_LH_RR → E30_door_LH_RR | 17건 | 16건 | 좌측 후면 도어 간 형상 유사 |
| E38_door_LH_RR → E30_door_LH_RR | 12건 | 16건 | 후면 도어 38/30톤 간 유사 |
| E25_door_RH → E30_E38_door_RH | 6건 | 5건 | 우측 도어 간 형상 유사 |
| E38_door_LH_FRT → E30_door_LH_FRT | 5건 | 4건 | 좌측 전면 도어 30/38톤 간 유사 |
...
```mermaid
graph LR
    A["Test 분할<br/>100%<br/>(377장)"] --> B["전경만 증강<br/>86.21%<br/>(377장)"]
    A --> C["전체 증강<br/>86.21%<br/>(377장)"]
    
    B -. "정확도 동일<br/>오분류 패턴 상이" .-> C
```

> **그림 5-7.** 증강 범위별 정확도 비교 (Test 분할 기준)

1. **Test 분할 대비 약 14%p 하락**: 온라인 증강만으로 학습한 모델이 오프라인 증강 수준의 색상 변형에는 상당히 취약함을 확인. 모델이 **RGB 텍스처 패턴에 강하게 의존**하고 있음을 시사
2. **배경 의존도 미미**: 전경만 증강(86.21%)과 전체 증강(86.21%)의 전체 정확도가 동일하여, 모델이 배경 패턴에 의존하지 않음을 확인. 다만 클래스별 세부 패턴은 상이
3. **취약 클래스 일관성**: E25_door_LH_RR은 두 조건 모두에서 가장 낮은 정확도(64~66%)를 기록하여, E30_door_LH_RR과의 혼동이 심각
4. **후면 도어 혼동 심화**: E38_door_LH_RR → E30_door_LH_RR 오분류가 두드러지며, 텍스처 변형 시 후면 도어 간 구분이 어려움을 확인
