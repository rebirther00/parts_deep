...
> **표 5-9.** 실시간 추론 파이프라인 검증

#### 5.3.5 모델 일반화 실험 (Generalization vs Memorization)

모델이 부품의 **형상적 특징(shape)**을 학습했는지, 아니면 특정 개체의 **표면 패턴(RGB texture)**을 암기했는지 검증하기 위한 체계적 실험을 수행하였다. 오프라인 증강 도구(`06_generate_augmented_dataset.py`)를 활용하여 RGB 텍스처를 의도적으로 변형한 데이터셋으로 원본 학습 모델을 평가하였다.
...
극단적 증강에서 확인된 텍스처 의존성을 더 세밀하게 분석하기 위해, 증강 강도를 moderate로 완화하고, 증강 범위(전경만 vs 전체)를 분리하여 실험하였다.
